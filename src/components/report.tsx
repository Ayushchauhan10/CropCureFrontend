import React, { useState } from 'react';
import { View, Text, StyleSheet, Image } from 'react-native';
import CheckBox from '@react-native-community/checkbox';

const Report = ({data} ) => {
  const { patientName, disease, symptoms, date, time, location, prescription } = data;
  const [selectedCheckboxes, setSelectedCheckboxes] = useState([false, false]);

  const toggleCheckbox = (index) => {
    const updatedCheckboxes = [...selectedCheckboxes];
    updatedCheckboxes[index] = !updatedCheckboxes[index];
    setSelectedCheckboxes(updatedCheckboxes);
  };
  return (
    <View style={styles.container}>

      <View style={styles.infoContainer}>
        <View style={{ alignItems: 'center' }}>
          <Image
            style={{ width: 100, height: 100, marginBottom: 20, borderRadius: 100, borderColor: 'black' }}
            source={{ uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRcJgbrT4WBPdXVO4f9ribR9CqkvFAZfr8J_JyIPPtyjWrH3VvQAnv4N0MPDfYjVrDXG3k&usqp=CAU' }}
          />
        </View>
        <Text style={styles.textContain}>
          <Text style={styles.labels}>Patient Name:</Text> {patientName}
        </Text>
        <Text style={styles.textContain}>
          <Text style={styles.labels}>Disease:</Text> {disease}
        </Text>
        <Text style={styles.textContain}>
          <Text style={styles.labels}>Symptoms:</Text> {symptoms}
        </Text>
        <Text style={styles.textContain}>
          <Text style={styles.labels}>Date:</Text> {date}
        </Text>
        <Text style={styles.textContain}>
          <Text style={styles.labels}>Time:</Text> {time}
        </Text>
        <Text style={styles.textContain}>
          <Text style={styles.labels}>Location:</Text> {location}
        </Text>
        <Text style={styles.textContain}>
          <Text style={styles.labels}>Prescription:</Text> {prescription}
        </Text>
      </View>

      <View style={{ width: '85%', backgroundColor: 'skyblue', paddingHorizontal: '5%', paddingVertical: '3%', borderRadius: 25, flexDirection: 'row', gap: 10, justifyContent: 'center' }}>
        <Text style={{ color: 'black', fontWeight: 'bold', fontSize: 16 }}>Download Detailed Report </Text>
        <Image
          style={{ width: 22, height: 22 }}
          source={{ uri: 'data:image/png;base64,' }}
        />
      </View>

      <View style={styles.checkboxContainer}>
        <View style={styles.checkboxRow}>
          <CheckBox
            value={selectedCheckboxes[0]}
            onValueChange={() => toggleCheckbox(0)}
            style={styles.checkbox}
          />
          <Text style={styles.label}>Approved</Text>
        </View>
        <View style={styles.checkboxRow}>
          <CheckBox
            value={selectedCheckboxes[1]}
            onValueChange={() => toggleCheckbox(1)}
            style={styles.checkbox}
          />
          <Text style={styles.label}>Not Approved</Text>
        </View>
        <View style={styles.checkboxRow}>
          <CheckBox
            value={selectedCheckboxes[2]}
            onValueChange={() => toggleCheckbox(2)}
            style={styles.checkbox}
          />
          <Text style={styles.label}>Put To wait</Text>
        </View>
      </View>

      <View style={styles.chatContainer}>
        {/* <View> */}
          <Text>Chat</Text>
          <Image
                source={{uri:'data:image/png;base64,'}}
                style={{ width: 30, height: 30 ,borderRadius:50,}}
          />
          {/* <Text>
                  Hello
                </Text> */}


        {/* </View> */}

      </View>

      <View style={styles.barContainer}>
          <Image
                source={{uri:'data:image/jpeg;base64,'}}
                style={{ width: 30, height: 30 ,borderRadius:50,}}
          />
          <Image
                source={{uri:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcToO8xulPc2-VZXb2h7XwjyRpZOsZJbIz0dLQ&usqp=CAU'}}
                style={{ width: 30, height: 30 ,borderRadius:50,}}
          />
          <Image
    source={{uri:'data:image/png;base64,'}}
                style={{ width: 30, height: 30 ,borderRadius:30,}}
          />
        
        
      </View>
    </View>
  );
};

const styles = StyleSheet.create({
  container: {
    padding: '12%',
    //  marginTop:'10%',
    backgroundColor: 'rgba(230,230,230,1)',
    height: '100%',
    flexDirection: 'column',
    gap: 20,
    alignItems: 'center',
    width: '100%',
  },

  mainInfoContainer: {

  },
  textContain: {
    fontSize: 16,
    fontWeight: 'bold'
  },
  labels: {
    fontWeight: 'bold',
    color: 'black',
    fontSize: 17,
    // flex: 1, // Take up half of the available space
  },
  infoContainer: {
    backgroundColor: 'skyblue',
    paddingVertical: '10%',
    paddingHorizontal: '5%',
    flexDirection: 'column',
    gap: 5,
    borderRadius: 15,
    // alignItems:'center',
    // justifyContent: 'center'
    // flex: 1, // Take up half of the available space
  },
  checkboxContainer: {
    flexDirection: "column",
    // : '10%,
    paddingHorizontal: '5%',
    paddingVertical: '2%',
    borderRadius: 20,
    marginBottom: 20,
    gap: 10,
    width: '100%',
    // backgroundColor:'black',
    elevation: 10,
    backgroundColor: 'white',
    shadowColor: 'black',
    shadowRadius: 20,

  },
  checkboxRow: {
    // backgroundColor:'#fff',
    flexDirection: "row",
    // justifyContent: "space",

    alignItems: 'center',
    // width:'fit',
    // width: 10,

  },
  checkbox: {
    alignSelf: "center"
  },
  label: {
    margin: 0
  },
  chatContainer: {
    // margin: 0
    width: '33%',
    backgroundColor: 'skyblue',
    paddingHorizontal: '10%',
    paddingVertical: '3%',
    position: "absolute",
    bottom: '8%',
    right: '10%',
    flexDirection: 'row',
    alignContent:'space-between',
    justifyContent:'center',
    alignItems:'center',
    gap:10,
    borderRadius:15,

  },
  barContainer:{
    backgroundColor:'rgba(128,128,128,0.5)',
    position: 'absolute',
    bottom:0,
    right:0,
    left:0,
    flexDirection:'row',
    height:'6.5%',
    alignItems:'center',
    justifyContent:'space-around',
    paddingHorizontal:'8%'
  }

});

export default Report;
